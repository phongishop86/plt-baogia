...
content = content.replace(
  ...
  "import { Plus, X, Pencil, Trash2, Briefcase, Users, FileText, ChevronLeft, Calendar, Printer, LayoutDashboard, MapPin, Wallet, Upload, Download, Info } from 'lucide-react';"
);
...
const pdStart = content.indexOf("function ProjectDetail(");
const pdShowModalStart = content.indexOf("const [showModal, setShowModal] = useState(false);", pdStart);
const pdSaveContractEnd = content.indexOf("setShowModal(false);\n  };\n", pdShowModalStart) + "setShowModal(false);\n  };\n".length;

if (pdShowModalStart !== -1 && pdSaveContractEnd !== -1) {
  content = content.substring(0, pdShowModalStart) + content.substring(pdSaveContractEnd);
}
