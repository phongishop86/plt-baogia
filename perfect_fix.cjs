...
lines = lines.map(line => {
  if (line.includes("import PizZip from 'pizzip';")) return "";
  if (line.includes("import Docxtemplater from 'docxtemplater';")) return "";
  if (line.includes("import { saveAs } from 'file-saver';")) return "import ProjectUnitsTab from './ProjectUnitsTab';";
  
  if (line.includes("import { db, type Project, type Personnel, type ProjectContract } from '../db/db';")) {
    return "import { db, type Project, type Personnel } from '../db/db';";
  }
  if (line.includes("import { Plus, X, Pencil, Trash2, Briefcase, Users, FileText, ChevronLeft, Calendar, Printer, LayoutDashboard, MapPin, Wallet, Upload, Download } from 'lucide-react';")) {
  ...
  // Update ProjectDetail variables
  if (line.includes("const contracts = useLiveQuery(() => db.projectContracts.where('projectId').equals(projectId).toArray());")) return "";
  if (line.includes("const allPersonnel = useLiveQuery(() => db.personnel.toArray());")) return "";
  if (line.includes("const [detailTab, setDetailTab] = useState<'DASHBOARD' | 'UNITS' | 'CONTRACTS' | 'EXPENSES'>('DASHBOARD');")) {
    return "  const [detailTab, setDetailTab] = useState<'DASHBOARD' | 'UNITS' | 'TEMPLATES' | 'EXPENSES'>('DASHBOARD');";
  }

  // Update totalExpense
  if (line.includes("const totalContractExpense = contracts?.reduce((sum, c) => sum + c.amount, 0) || 0;")) return "";
  if (line.includes("const totalOtherExpense = expenses?.reduce((sum, e) => sum + e.amount, 0) || 0;")) return "";
  if (line.includes("const totalExpense = totalContractExpense + totalOtherExpense;")) {
    return "  const totalExpense = expenses?.reduce((sum, e) => sum + e.amount, 0) || 0;";
  }

  // Button text
  if (line.includes("setDetailTab('CONTRACTS')")) {
    return line.replace(/CONTRACTS/g, "TEMPLATES");
  }
  if (line.includes("Hợp đồng Nhân sự")) {
    if (line.includes("<FileText")) { // Only in the tab
      return line.replace("Hợp đồng Nhân sự", "Biểu mẫu Dự án");
    }
  }
...
if (showModalIndex !== -1 && computedProgressIndex !== -1) {
  lines = [...lines.slice(0, showModalIndex), "  if (!project) return <div>Đang tải...</div>;", ...lines.slice(computedProgressIndex)];
}
