...
// 1. Replace the block {detailTab === 'CONTRACTS' && ( ... )} with {detailTab === 'TEMPLATES' && <ProjectTemplatesTab projectId={projectId} />}
const startIndex = content.indexOf("{detailTab === 'CONTRACTS' && (");
if (startIndex !== -1) {
  // Find the end of this block. It ends just before `{detailTab === 'UNITS'`
  const endIndex = content.indexOf("{detailTab === 'UNITS'", startIndex);
  if (endIndex !== -1) {
    const before = content.substring(0, startIndex);
    const after = content.substring(endIndex);
    content = before + "{detailTab === 'TEMPLATES' && <ProjectTemplatesTab projectId={projectId} />}\n\n      " + after;
  }
}

// 2. Remove function ProjectUnitsTab completely since it's now imported
...
if (unitsTabStart !== -1) {
  ...
  if (expensesTabStart !== -1) {
    const before = content.substring(0, unitsTabStart);
    const after = content.substring(expensesTabStart);
    content = before + after;
  }
}

// 3. Append function ProjectTemplatesTab at the end
