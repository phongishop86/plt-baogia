...
// We need to remove from "const [showModal, setShowModal] = useState(false);" (inside ProjectDetail)
// up to "  let computedProgress = 0;"
...
if (showModalIndex !== -1 && computedProgressIndex !== -1) {
  content = [...lines.slice(0, showModalIndex), ...lines.slice(computedProgressIndex)].join('\n');
}
