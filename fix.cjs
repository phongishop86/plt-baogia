...
// 2. Change detailTab type and initial state
content = content.replace(
...
// 3. Change CONTRACTS tab button to TEMPLATES
content = content.replace(
...
// 4. Find where CONTRACTS tab block starts and ends.
const contractsUIStart = content.indexOf("{detailTab === 'CONTRACTS' && (");
const contractsUIEnd = content.indexOf("{detailTab === 'UNITS' && <ProjectUnitsTab projectId={projectId} />}");
if (contractsUIStart !== -1 && contractsUIEnd !== -1) {
  content = content.substring(0, contractsUIStart) + 
            "{detailTab === 'TEMPLATES' && <ProjectTemplatesTab projectId={projectId} />}\n      " + 
            content.substring(contractsUIEnd);
}

// 5. Remove unused states in ProjectDetail
// We need to keep detailTab.
const projectDetailStart = content.indexOf("function ProjectDetail(");
const computedProgressStart = content.indexOf("let computedProgress = 0;", projectDetailStart);

if (projectDetailStart !== -1 && computedProgressStart !== -1) {
  const codeBefore = content.substring(0, projectDetailStart);
  let detailCode = content.substring(projectDetailStart, computedProgressStart);
  const codeAfter = content.substring(computedProgressStart);

  // Remove contracts and allPersonnel useLiveQuery
  detailCode = detailCode.replace(/const contracts = useLiveQuery[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const allPersonnel = useLiveQuery[^\n]*\n/g, '');

  // Remove state variables
  detailCode = detailCode.replace(/const \[showModal, setShowModal\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[editingId, setEditingId\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[personnelId, setPersonnelId\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[startDate, setStartDate\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[endDate, setEndDate\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[unit, setUnit\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[quantity, setQuantity\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[unitPrice, setUnitPrice\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[taxRateTNCN, setTaxRateTNCN\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[jobDescription, setJobDescription\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[location, setLocation\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[deviceQuantity, setDeviceQuantity\] = useState[^\n]*\n/g, '');
  detailCode = detailCode.replace(/const \[exportingId, setExportingId\] = useState[^\n]*\n/g, '');

  // Remove handleExportWord
  detailCode = detailCode.replace(/const handleExportWord = async \([\s\S]*?finally {\s*setExportingId\(null\);\s*}\s*};\n/g, '');

  // Remove openAddModal, openEditContract, saveContract
  detailCode = detailCode.replace(/const openAddModal = \(\) => {[\s\S]*?setShowModal\(true\);\s*};\n/g, '');
  detailCode = detailCode.replace(/const openEditContract = \([\s\S]*?setShowModal\(true\);\s*};\n/g, '');
  detailCode = detailCode.replace(/const saveContract = async \([\s\S]*?setShowModal\(false\);\s*};\n/g, '');

  content = codeBefore + detailCode + codeAfter;
}

// 6. Fix total expense calculation
content = content.replace(
